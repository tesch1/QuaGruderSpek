...
J13 = 0; % in Hz, per convention
J23 = 5; % in Hz, per convention

% Hamiltonians for the coupled 2-spin experiment
Hcs1 = omega1 * IZEE;
Hcs2 = omega2 * IEZE;
Hcs3 = omega3 * IEEZ;
Hfree = Hcs1 + Hcs2 + Hcs3 + 2*pi*J12*IZZE + 2*pi*J13*IZEZ + 2*pi*J23*IEZZ;
...
% record M for ~1s
U = makeU(Hfree, dt);
U90x = makeU(pi/2*(IXEE+IEXE+IEEX), 1);
...
for ti1=1:np
  ...
  for ti2=1:np
    % take measurement
    tcur = t1 + ti2*dt;
    m1(ti1,ti2) = meas3(rho) * exp((-tcur) / T2);
